...
void main() {
  ...
  float len = length(p);
  vec2 ripple = uv + p/len*0.03*cos(len*20.0-u_time*4.0);
  float delta = (sin(mod(u_time, u_duration) * (2.0*PI/u_duration) + 1.0)/2.0);
  vec2 uv2 = mix(ripple, uv, 0.0);
  vec3 color = texture2D(tex0, uv2).rgb;
  vec3 add = vec3(sin(cos(u_time+3.0 * uv2.y)*2.0 / uv2.x),cos(sin(u_time+2.0 / uv2.x) * 3.0 * mod(uv2.y , u_time)),0.0);

  //gl_FragColor = tex;
  gl_FragColor = vec4(mix(color, add, 0.5), 1.0);
}
